...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await request.json() as GenerateLoreRequest;
    ...
    const sanitizedDescription = body.description.trim().replace(/[<>]/g, '');

    // Call Gemini API for lore generation
    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-05-20:generateContent?key=${GEMINI_API_KEY}`;
    const prompt = `Based on this description: "${sanitizedDescription}", write a short, mythical-sounding lore or backstory for this creation. Make it sound like a legend.`;
    ...
    console.error('Lore generation error:', error instanceof Error ? error.message : error);
